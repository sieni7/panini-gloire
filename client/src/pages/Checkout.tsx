...
export default function Checkout() {
  ...
  return <main className="checkout-page"><Link href="/" className="back-link"><ArrowLeft size={17} /> Retour au menu</Link><div className="checkout-layout"><section><p className="eyebrow dark">DERNIÈRE LIGNE DROITE</p><h1>On prépare votre pause.</h1><p className="checkout-intro">Laissez vos coordonnées, Marina reçoit le récapitulatif sur WhatsApp.</p><form onSubmit={submit} className="checkout-form"><label>Votre nom<input value={name} onChange={(event) => setName(event.target.value)} placeholder="Ex. Aïcha Kouamé" autoComplete="name" /></label><label>Téléphone ivoirien<input value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="+225 05 74 97 10 22" inputMode="tel" autoComplete="tel" /></label><fieldset><legend>Comment vous recevez votre commande ?</legend><label className="choice"><input type="radio" checked={type === "sur_place"} onChange={() => setType("sur_place")} /> <span><strong>Sur place</strong><small>Adjamé Bingerville · <a className="map-link" href="https://www.google.com/maps/search/?api=1&query=Panini+de+la+Gloire+Adjam%C3%A9+Bingerville+En+face+de+BonPrix" target="_blank" rel="noreferrer">Voir sur Google Maps</a></small></span></label><label className="choice"><input type="radio" checked={type === "livraison"} onChange={() => setType("livraison")} /> <span><strong>Livraison</strong><small>On vous appelle pour préciser</small></span></label></fieldset>{type === "livraison" && <label>Adresse<input value={address} onChange={(event) => setAddress(event.target.value)} placeholder="Quartier, rue, repère" autoComplete="street-address" /></label>}{error && <p className="form-error" role="alert">{error}</p>}{type === "livraison" && <p className="delivery-fee-note" role="note">Les frais de livraison sont à régler directement au livreur.</p>}<button className="primary-button submit-button" type="submit"><MessageCircle size={18} /> Préparer WhatsApp <ArrowRight size={18} /></button></form></section><aside className="order-summary"><p className="eyebrow dark">RÉCAPITULATIF</p><h2>Votre sélection</h2>{cart.map((item) => <div className="summary-line" key={item.name}><span>{item.quantity} × {item.name}</span><strong>{formatCFA(item.price * item.quantity)}</strong></div>)}<div className="summary-total"><span>Total</span><strong>{formatCFA(total)}</strong></div><p className="location-note"><MapPin size={17} /> Adjamé Bingerville<br />En face de BonPrix</p></aside></div></main>;
}
