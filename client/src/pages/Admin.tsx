...
type Site = {
  brandName: string; locationLabel: string; address: string; phone: string; heroEyebrow: string;
...
const initialSite: Site = {
  brandName: "Panini de la Gloire", locationLabel: "ADJAMÉ BINGERVILLE • EN FACE DE BONPRIX", address: "Adjamé Bingerville • En face de BonPrix", phone: "+2250574971022", heroEyebrow: "FAIT MINUTE, LIVRÉ AVEC LE SOURIRE", heroTitle: "La pause qui mérite une ovation.", heroDescription: "Choisissez votre recette, composez votre panier et confirmez votre commande directement sur WhatsApp.", serviceNote: "Paiement à la livraison", openingHours: "Tous les jours · 10h00 — 22h00", deliveryNote: "Livraison selon zone et disponibilité", whatsappTemplate: "Nouvelle commande — Panini de la Gloire",
};
...
export default function Admin() {
  ...
  const [active, setActive] = useState<"products" | "site" | "categories">("products");
    ...
    <main className="admin-page">
      ...
      <div className="admin-layout">
        ...
        <section className="admin-content">
          ...
          {active === "categories" && <div className="category-manager"><div className="admin-heading"><div><p className="eyebrow dark">ORGANISATION</p><h2>Les catégories</h2></div></div><div className="category-add-row"><input value={categoryName} onChange={(event) => setCategoryName(event.target.value)} placeholder="Nouvelle catégorie" aria-label="Nom de la nouvelle catégorie" /><button className="primary-button" type="button" onClick={addCategory}>Ajouter</button></div>{categoryError && <p className="form-error" role="alert">{categoryError}</p>}<div className="category-list">{[...categories].sort((a, b) => a.sortOrder - b.sortOrder).map((item) => <div className="category-row" key={item.id}><input value={item.name} onChange={(event) => renameCategory(item.id, event.target.value)} aria-label={`Nom de la catégorie ${item.name}`} /><button className="delete-button" type="button" onClick={() => deleteCategory(item.id)} aria-label={`Supprimer la catégorie ${item.name}`}><Trash2 size={17} /></button></div>)}</div><p className="admin-help">Une catégorie qui contient encore un produit ne peut pas être supprimée. Changez d’abord la catégorie de ses produits.</p></div>}

          {active === "products" && <><div className="admin-heading"><div><p className="eyebrow dark">CATALOGUE</p><h2>Les recettes du jour</h2></div><div className="admin-heading-actions"><button className="secondary-button" type="button" onClick={addProduct}>Nouveau produit</button><button className="primary-button" onClick={saveDraft}>{saved ? <><Check size={17} /> Brouillon enregistré</> : <><Save size={17} /> Enregistrer le brouillon</>}</button></div></div><div className="admin-workspace"><div className="admin-list">{products.map((item) => <button className={selectedId === item.id ? "admin-product-row active" : "admin-product-row"} key={item.id} onClick={() => setSelectedId(item.id)}><span className="admin-thumb" style={{ backgroundImage: `url(${item.image})` }} /><span><strong>{item.name}</strong><small>{item.price} F · {item.available ? "Disponible" : "Rupture"}</small></span><span className={item.available ? "status-dot" : "status-dot off"} /></button>)}</div><div className="admin-form-card"><div className="admin-form-heading"><div><p className="eyebrow dark">FICHE PRODUIT</p><h3>{selected.name}</h3></div><button className="delete-button" type="button" onClick={deleteProduct} aria-label="Supprimer le produit"><Trash2 size={17} /></button></div>{formError && <p className="form-error" role="alert">{formError}</p>}<label>Nom du produit<input required value={selected.name} onChange={(event) => { setFormError(""); updateProduct({ name: event.target.value }); }} /></label><label>Description<textarea value={selected.description} onChange={(event) => updateProduct({ description: event.target.value })} rows={3} /></label><div className="admin-form-grid"><label>Prix (FCFA)<input required type="number" min={50} step={50} value={selected.price} onChange={(event) => { setFormError(""); updateProduct({ price: Number(event.target.value) }); }} /></label><label>Catégorie<select required value={selected.category} onChange={(event) => updateProduct({ category: event.target.value })}>{categories.map((item) => <option key={item.id}>{item.name}</option>)}</select></label></div><label className="admin-check"><input type="checkbox" checked={selected.available} onChange={(event) => updateProduct({ available: event.target.checked })} /> Produit disponible sur la carte</label><div className="upload-zone"><ImagePlus size={24} /><strong>Remplacer la photo</strong><span>JPG, PNG ou WebP · aperçu local immédiat</span><label className="upload-button"><UploadCloud size={16} /> Choisir une image<input type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => handleImage(event.target.files?.[0])} /></label></div></div></div></>}

          {active === "site" && <><div className="admin-heading"><div><p className="eyebrow dark">IDENTITÉ ET SERVICE</p><h2>Les informations visibles</h2></div><button className="primary-button" onClick={saveDraft}><Save size={17} /> Enregistrer le brouillon</button></div><div className="site-form-card">{Object.entries(site).map(([key, value]) => <label key={key}>{key === "brandName" ? "Nom de l’enseigne" : key === "locationLabel" ? "Libellé de localisation" : key === "address" ? "Adresse" : key === "phone" ? "WhatsApp" : key === "openingHours" ? "Horaires" : key === "heroEyebrow" ? "Sur-accroche" : key === "heroTitle" ? "Accroche principale" : key === "heroDescription" ? "Description hero" : key === "serviceNote" ? "Note de service" : key === "deliveryNote" ? "Note livraison" : "Modèle WhatsApp"}{key === "heroDescription" ? <textarea rows={4} value={value} onChange={(event) => setSite({ ...site, [key]: event.target.value })} /> : <input value={value} onChange={(event) => setSite({ ...site, [key]: event.target.value })} />}</label>)}</div></>}
